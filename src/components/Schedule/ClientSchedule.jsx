...
import { format, startOfWeek, addDays, startOfMonth, endOfMonth, eachDayOfInterval, isBefore, isEqual } from "date-fns";
...
export default function ClientSchedule() {
    ...
    const today = new Date(); // Current date: March 18, 2025
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.log("Schedules from API:", scheduleResponse.data);
    ...
    const getWeekSlots = () => {
        ...
        days: DAYS.map((_, dayIndex) => {
            ...
            return {
            available: !isPast && (!schedule || schedule.status !== 1), // Only available if not past and not booked
    ...
    const getMonthSlots = () => {
        ...
        return {
            date,
            available: !isPast && !daySchedules.some(s => s.status === 1), // Only available if not past and not booked
            isPast: isPast,
        };
        });
    };
    ...
    const renderWeekSlots = () => {
        ...
        <div key={hour} className="slot-row flex items-center mb-2">
            <div className="time-label w-20 text-center font-semibold text-gray-700">
            {hour}
            </div>
    ...
    const renderMonthDays = () => {
        ...
        <div className="month-grid grid grid-cols-7 gap-1">
            ...
            return (
                ...
                onClick={() => isCurrentMonth && !isPast && availability?.available && handleDaySelect(date)}
                disabled={!isCurrentMonth || isPast || !availability?.available}
                >
                {format(date, "d")}
                </button>
            );
        ...
        <div className="header flex justify-between items-center mb-6">
            ...
            {viewMode === "week" ? (
                ...
                <span className="text-lg font-semibold text-gray-700">
                    ...
                    {format(addDays(selectedWeek, 6), "MMM d, yyyy")}
                </span>
                <button
                    className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600"
                    onClick={() => {
        ...
        {viewMode === "week" ? (
            ...
            {isConfirmed && (
                ...
                <p className="text-gray-700">
                    ...
                    {format(addDays(selectedWeek, selectedSlot.dayIndex), "MMM d")} at{" "}
                    {selectedSlot.hour}.
                </p>
                <button
